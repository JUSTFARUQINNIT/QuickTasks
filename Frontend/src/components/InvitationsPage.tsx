...
export function InvitationsPage() {
  ...
  useEffect(() => {
    ...
    async function load(user: User) {
      try {
        ...
        const [byUserIdPendingSnap, byEmailPendingSnap, byUserIdAcceptedSnap, byEmailAcceptedSnap, sentSnap] =
          await Promise.all([
            getDocs(
              query(
                invitesRef,
        ...
        const receivedDocs = [...byUserIdPendingSnap.docs, ...byEmailPendingSnap.docs].filter((d) => {
        ...
        const acceptedDocs = [...byUserIdAcceptedSnap.docs, ...byEmailAcceptedSnap.docs].filter((d) => {
        ...
        if (receivedBase.length === 0 && acceptedBase.length === 0 && sentBase.length === 0) {
          setReceivedInvites([])
          setAcceptedInvites([])
          setSentInvites([])
          setLoading(false)
        ...
        await Promise.all(
          inviterIds.map(async (uid) => {
            try {
              ...
              const data = inviterSnap.data() as { email?: string | null; name?: string | null }
        ...
        setAcceptedInvites(
          acceptedBase.map((invite) => ({
            ...invite,
            invitedByLabel:
              inviterLabels.get(invite.invitedBy) ?? invite.invitedByEmail ?? invite.invitedBy,
          })),
        )
    ...
    try {
      ...
      setAcceptedInvites((prev) => [
        ...prev,
        {
          ...
          status: 'accepted',
          invitedByLabel: (invite as TaskInviteWithLabel).invitedByLabel ?? invite.invitedBy,
        },
      ])
  ...
  async function handleDecline(invite: TaskInvite) {
    try {
      ...
      const message = err instanceof Error ? err.message : 'Could not decline invitation.'
      setError(message)
    } finally {
      setActioningId(null)
    }
    ...
    <div className="tasks-shell tasks-shell--tasks">
      <section className="tasks-panel tasks-list-panel">
        <div className="tasks-list-header">
          ...
          <p className="tasks-subtitle">Review collaboration invites for your tasks.</p>
        </div>
        ...
        {loading ? (
        ...
        ) : receivedInvites.length === 0 && acceptedInvites.length === 0 && sentInvites.length === 0 ? (
          <div className="tasks-empty">
            <p>No invitations.</p>
          </div>
        ) : (
          <>
            {receivedInvites.length > 0 && (
              <>
                <h3 className="tasks-heading" style={{ marginTop: 24 }}>Invitations for you</h3>
                    ...
                    <li key={invite.id} className="task-item">
                      ...
                      <div className="task-card-body">
                        <div className="task-card-row">
                          ...
                          <span className="task-card-value">{invite.invitedByLabel}</span>
                        </div>
                        <div className="task-card-row">
                          <span className="task-card-label">Your email</span>
                          <span className="task-card-value">{invite.invitedEmail}</span>
                        </div>
                      </div>
                      <div className="invitation-form-actions" style={{ marginTop: 16 }}>
                        <button
                          type="button"
            ...
            {acceptedInvites.length > 0 && (
              <>
                <h3 className="tasks-heading" style={{ marginTop: 32 }}>Accepted invitations</h3>
                <ul className="invitation-card-list">
                  {acceptedInvites.map((invite) => (
                    <li key={invite.id} className="task-item">
                      <div className="task-card-header">
                        ...
                        <span className="task-status task-status--completed task-status--pill">Accepted</span>
                      </div>
                      <div className="task-card-body">
                        <div className="task-card-row">
                          <span className="task-card-label">Invited by</span>
            ...
            {sentInvites.length > 0 && (
              <>
                <h3 className="tasks-heading" style={{ marginTop: 32 }}>Invitations you sent</h3>
                <ul className="invitation-card-list">

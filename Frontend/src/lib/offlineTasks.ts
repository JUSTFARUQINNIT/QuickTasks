...
export function readCachedTasks<T>(): T[] {
  ...
  const fromOldKey = safeJsonParse<T[]>(localStorage.getItem(LEGACY_CACHED_TASKS_KEY))
...
export function readOfflineQueue(): OfflineTaskAction[] {
  const parsed = safeJsonParse<OfflineTaskAction[]>(localStorage.getItem(OFFLINE_QUEUE_KEY))
...
export function enqueueOfflineAction(action: Omit<OfflineTaskAction, 'id' | 'createdAt'>) {

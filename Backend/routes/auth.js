...
const APP_URL = process.env.APP_URL ?? "http://localhost:5173/reset-password"
...
router.post("/reset-password", async (req, res) => {
  try {
    ...
    if ((data.email ?? "").toLowerCase() !== email) {
      return res.status(400).json({ error: "Reset link does not match this email address." })
    }

    const uid = data.uid
    if (!uid) {
      return res.status(500).json({ error: "Reset link is missing account information." })
    }
    ...
    const errMsg = e instanceof Error ? e.message : String(e)
    return res.status(500).json({ error: `Failed to reset password: ${errMsg}` })
  }
})
...
router.post("/welcome", async (req, res) => {
  try {
      ...
      <table role="presentation" cellspacing="0" cellpadding="0" border="0" align="center" width="100%" style="max-width:520px;margin:0 auto;background:#020617;border-radius:18px;border:1px solid #1f2937;box-shadow:0 18px 45px rgba(15,23,42,0.9);">
        ...
        <tr>
          <td style="padding:24px 28px 8px;">
            ...
            <img src="/quicktasks-logo.svg" alt="QuickTasks logo" style="width: 27px; height: auto;">
            <h2 style="font-size:22px;line-height:1.3;color:#f9fafb;font-weight:600;">QuickTasks</h2>
            </div>
            </div>
    ...
    const errMsg = e instanceof Error ? e.message : String(e)
    return res.status(500).json({ error: `Failed to send welcome email: ${errMsg}` })
  }
})
